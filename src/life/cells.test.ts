import { boundsFrom, vec, ZERO } from '@conway/geom'
...
describe('clipAlive', () => {
  it('keeps cells inside a half-open window', () => {
    ...
    const clipped = clipAlive(alive, boundsFrom(ZERO, vec(5, 5)))
  ...
  it('returns empty when nothing intersects', () => {
    ...
    expect(clipAlive(alive, boundsFrom(ZERO, vec(5, 5))).size).toBe(0)
  })
})

...
/** Pin the pattern in the viewport so only declared velocity shows as motion. */
...
function renderCells(item) {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      cell.className = alive.has(pack(x, y)) ? "cell alive" : "cell";
      frag.append(cell);
    }
  }
...
/** Scroll the grid opposite the ship's travel (treadmill). */
